...
export default function AdminTopbar({ user, query, setQuery, onLogout }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex min-h-20 items-center justify-between gap-4 border-b border-slate-200 bg-white/95 px-4 backdrop-blur-xl sm:px-7">
      <div className="hidden md:block"><h1 className="text-xl font-black text-slate-950">Admin Dashboard</h1><p className="text-xs text-slate-500">Manage your restaurant, delight your customers.</p></div>
      <label className="relative min-w-0 flex-1 md:max-w-md">
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search dashboard actions…" className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm text-slate-900 outline-2 outline-transparent outline-offset-1 placeholder:text-slate-400 focus-visible:border-teal-600 focus-visible:outline-teal-600" />
      </label>
